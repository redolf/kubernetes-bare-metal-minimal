...
| kubeProxyReplacement=true                                           | Заменяет kube-proxy и берёт на себя обработку kubernetes-сервисов (ClusterIP, NodePort, LoadBalancer). <br><br>Маршрутизация и балансировка нагрузки реализуются на уровне ядра Linux с использованием eBPF (без традиционных iptables).                                                                        |
...
| loadBalancer.mode=snat                                              | Подменяет исходный IP-адрес пакетов на IP-адрес узла (ноды), с которого уходит трафик. <br><br>Это позволяет обходить проблемы с асимметричной маршрутизацией (например, когда ответный трафик не возвращается на тот же узел).                                                                                 |
| loadBalancer.bpfSocketLBHostnsOnly=true                             | Ограничение работы BPF-балансировщика только хостовым неймспейсом:<br>- Cilium использует BPF-балансировщик (на основе eBPF) для распределения трафика между подами.<br>- Параметр hostnsOnly=true означает, что балансировщик работает только в хостовом сетевом пространстве (не в пространстве контейнеров). |
| l2announcements.enabled=true                                        | L2-объявление IP-адресов:<br>Cilium объявляет IP-адреса LoadBalancer-сервисов на уровне L2 (Ethernet) через ARP/NDP.<br><br>Это позволяет внешним устройствам (маршрутизаторам, коммутаторам) узнавать, на каком узле кластера находится IP-адрес сервиса                                                       |
| l2announcements.policy.default-l2-announcement.interfaces[0]=eth0   | Интерфейс для ARP (eth0 → MAC ответы!)                                                                                                                                                                                                                                                                          |
| l2announcements.policy.default-l2-announcement.loadBalancerIPs=true | Объявлять LoadBalancer IP                                                                                                                                                                                                                                                                                       |
