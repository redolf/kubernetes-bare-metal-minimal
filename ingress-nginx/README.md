...
- Проверим что ingress контроллер работает задеплоив приложение с Nginx для теста из файла site-sample.yaml
  ...
  >
  > - Перейдите по адресу что указан в манифесте Ingress, в данном случае http://t1.poletaevlev.ru в браузере. Если всё хорошо, вы увидите страницу по умолчанию nginx, на которой он поприветствует вас: **Welcome to nginx!**. 
  > - Чтобы протестировать доступность симулякра frontend Web API, в браузере пытаемся посмотреть страницу по адресу http://t1.poletaevlev.ru/test. Ответом должен быть **JSON**: 
  > ```
